package com.woniukeji.demo.dao;
...
import com.woniukeji.demo.CityBean;
import com.woniukeji.demo.RegionInfo;


public class RegionDAO {
	
	public static List<RegionInfo> getProvincesOrCityOnId(int id){
	...
	public static List<CityBean> getCityOnParent(String parent){
		SQLiteDatabase db = SQLiteDatabase.openOrCreateDatabase(DBManager.DB_PATH + "/" + DBManager.DB_NAME, null);
		List<CityBean> regionInfos = new ArrayList<CityBean>();//String.valueOf(type)
		Cursor cursor = db.rawQuery("select name,parentAdco,citycode from city where parentAdco="+parent,null);
	
		while (cursor.moveToNext()) {
			CityBean regionInfo = new CityBean();
			String parent1 = cursor.getString(cursor
					.getColumnIndex("parentAdco"));
			String name = cursor.getString(cursor
					.getColumnIndex("name"));
			...
			regionInfo.setName(name);
			regionInfo.setCityCode(citycode);
			regionInfos.add(regionInfo);
		}
		cursor.close();
		db.close();
